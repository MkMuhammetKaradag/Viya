...
func (r *Repository) GetUserByID(ctx context.Context, userID uuid.UUID) (*domain.User, error) {
	query := `
		SELECT id, email, username, first_name, last_name, bio, location, website, preferences, avatar_url, created_at 
		FROM users 
	...
	err := r.db.QueryRowContext(ctx, query, userID).Scan(
		...
		&user.Bio, &user.Location, &user.Website, &prefsJSON, &user.AvatarURL, &user.CreatedAt,
	)
